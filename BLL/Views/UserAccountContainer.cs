...
using System.Linq;
using Sra.P2rmis.Bll.Interfaces;
...
namespace Sra.P2rmis.Bll.Views
{
    ...
    public class UserAccountContainer : IUserAccountContainer
    {
        ...
        /// <exception cref="">Any exceptions are passed up and should be caught by the calling method.</exception>
        public void UpdateUserInfo(string firstName, string lastName, string middleName, string fullUserName, int? prefixId, int? suffixLkpID)
        {
        ...
        private UserPhone GetPhoneNumber(bool state)
        {
            ...
            if (User != null)
            {
                result = User.UserInfoes.Single().UserPhones.Single(f => f.PrimaryFlag == state);
            }
        ...
        private UserEmail GetEMail(bool state)
        {
            ...
            if (User != null)
            {
                result = User.UserInfoes.Single().UserEmails.Single(f => f.PrimaryFlag == state);
            }
