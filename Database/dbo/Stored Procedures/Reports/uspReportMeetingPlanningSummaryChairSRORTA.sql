CREATE PROCEDURE [dbo].[uspReportMeetingPlanningSummaryChairSRORTA]
...
WITH 
	...
	select distinct 
		...
		, CASE WHEN ClientParticipantType.ReviewerFlag = 1 OR
           ClientParticipantType.ParticipantTypeAbbreviation = 'SRO' or ClientParticipantType.ParticipantTypeAbbreviation = 'RTA' THEN 1 ELSE 0 END AS  [Total Participant Count]
		,  CASE WHEN ViewPanelUserAssignment.ParticipationMethodId = 1 AND ClientParticipantType.ReviewerFlag = 1 OR
           ClientParticipantType.ParticipantTypeAbbreviation = 'SRO' THEN 1 ELSE 0 END AS [Total Onsite Participant Count]
		, CASE WHEN (ClientParticipantType.ReviewerFlag = 1 and ViewPanelUserAssignment.ParticipationMethodId = 1) and (ClientParticipantType.ConsumerFlag <> 1)  THEN 1 ELSE 0 END AS  [Total Onsite Reviewer]
		...
		, CASE WHEN (ClientParticipantType.ReviewerFlag = 1 and ViewPanelUserAssignment.RestrictedAssignedFlag = 1 
						and ViewPanelUserAssignment.ParticipationMethodId = 1) THEN 1 ELSE 0 END AS  [Total Onsite Adhoc Reviewer]
			...
			INNER JOIN ViewPanelUserAssignment ON ViewSessionPanel.SessionPanelId = ViewPanelUserAssignment.SessionPanelId
				...
				left join 
					(SELECT ViewPanelUserAssignment.SessionPanelID, 
							Chairperson = dbo.udfConcatString(ViewPanelUserAssignment.SessionPanelID,'CH') ,
							SRO = dbo.udfConcatString(ViewPanelUserAssignment.SessionPanelID,'SRO') ,
							RTA = dbo.udfConcatString(ViewPanelUserAssignment.SessionPanelID,'RTA') 
...
GRANT EXECUTE
    ON OBJECT::[dbo].[uspReportMeetingPlanningSummaryChairSRORTA] TO [NetSqlAzMan_Users]
    AS [dbo];