...
-- Description: Storeprocedure to Create Client Report ExpeditedReview
-- =============================================
CREATE PROCEDURE [dbo].[uspReportExpeditedReviewData] 
...
SELECT        ClientProgram.ProgramAbbreviation, ViewSessionPanel.PanelAbbreviation, ViewApplication.LogNumber, ViewClientElement.ElementAbbreviation, ViewMechanismTemplateElement.OverallFlag, ViewSessionPanel.PanelName, 
                         ViewMechanismTemplateElement.SortOrder, ViewClientAwardType.ClientAwardTypeID,ViewClientAwardType.AwardAbbreviation,ViewClientAwardType.AwardDescription, ViewApplicationWorkflowStep.StepName AS StepTypeName, ViewPanelApplicationReviewerAssignment.SortOrder AS Sort_Order, ViewProgramYear.Year, 
						 ...
						 CASE WHEN ViewMechanismTemplateElement.OverallFlag = 1 Then 1 
						 When ViewMechanismTemplateElement.OverallFlag<>1 THEN ViewMechanismTemplateElement.SortOrder END AS Element_Sort_Order,ViewApplicationWorkflowStep.ApplicationWorkflowStepId
...
WHERE        (ClientAssignmentType.AssignmentTypeId <> 8) AND (ClientAssignmentType.AssignmentTypeId <> 7) AND (ReviewStatus.ReviewStatusTypeId = 1) AND (ViewMechanismTemplateElement.ScoreFlag = 1) AND 
                         (ViewApplicationWorkflowStep.StepName='Revised') 
ORDER BY  ViewApplication.LogNumber, Element_Sort_Order
END
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[uspReportExpeditedReviewData] TO [NetSqlAzMan_Users]
    AS [dbo];
