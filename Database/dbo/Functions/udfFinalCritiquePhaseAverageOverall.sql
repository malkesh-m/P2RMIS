CREATE FUNCTION [dbo].[udfFinalCritiquePhaseAverageOverall]
(
...
(
	...
	WHERE  [dbo].[ViewPanelApplication].[PanelApplicationId] = @PanelApplicationId AND
	[dbo].[ViewMechanismTemplateElement].OverallFlag = 1 AND (ViewApplicationStage.ActiveFlag = 1) AND ViewApplicationStage.ReviewStageId IN (1,2)
) InnerQuery
WHERE Ranking = 1
GROUP BY InnerQuery.ClientElementId
GO
GRANT SELECT
    ON OBJECT::[dbo].[udfFinalCritiquePhaseAverageOverall] TO [NetSqlAzMan_Users]
    AS [dbo];
