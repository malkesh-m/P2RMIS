/*
Gets the last updated critique overall score information for a given application without including CR Scores (currently used by CPRIT RES - Not Discussed)
*/
CREATE FUNCTION [dbo].[udfLastUpdatedCritiquePhaseAverageOverallWoCRs]
(
...
(
	...
	[dbo].[ViewPanelUserAssignment] ON [dbo].[ViewApplicationWorkflow].PanelUserAssignmentId = [dbo].[ViewPanelUserAssignment].PanelUserAssignmentId INNER JOIN
	...
	[dbo].[ViewMechanismTemplateElement].OverallFlag = 1 AND dbo.ClientParticipantType.ConsumerFlag = 0
) InnerQuery
WHERE Ranking = 1
GROUP BY InnerQuery.ClientElementId
GO
GRANT SELECT
    ON OBJECT::[dbo].[udfLastUpdatedCritiquePhaseAverageOverallWoCRs] TO [NetSqlAzMan_Users]
    AS [dbo];